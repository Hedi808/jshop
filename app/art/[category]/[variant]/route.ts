const palettes: Record<string, [string, string, string]> = {
  electronique: ["#9dacb5", "#ff6500", "#101820"], smartphones: ["#a8b5c7", "#ff6500", "#111827"], informatique: ["#a4ada9", "#ff6500", "#17201d"], audio: ["#b8abbf", "#ff6500", "#1f1625"],
...
const labels: Record<string, string> = {
  electronique: "ÉLECTRONIQUE / IMAGE ET ÉNERGIE", smartphones: "SMARTPHONES / RESTEZ CONNECTÉ", informatique: "INFORMATIQUE / TRAVAILLEZ MIEUX", audio: "AUDIO / UN SON PLUS CLAIR",
  ...
  gaming: "GAMING / JOUEZ PRÉCIS", "objets-connectes": "OBJETS CONNECTÉS / MAÎTRISEZ VOTRE ESPACE", editorial: "JOSHOP / LA TECH AVANCE",
};

function phone(accent: string, ink: string) { return `<rect x="325" y="120" width="350" height="690" rx="62" fill="${ink}"/><rect x="350" y="165" width="300" height="585" rx="40" fill="${accent}" opacity=".78"/><circle cx="500" cy="780" r="13" fill="white"/><circle cx="410" cy="225" r="42" fill="${ink}"/><circle cx="510" cy="225" r="42" fill="${ink}"/>`; }
function laptop(accent: string, ink: string) { return `<rect x="210" y="180" width="580" height="390" rx="28" fill="${ink}"/><rect x="245" y="220" width="510" height="310" rx="10" fill="${accent}"/><path d="M145 590h710l-60 90H205l-60-90Z" fill="${ink}"/><path d="M430 605h140" stroke="white" stroke-width="12" opacity=".45"/>`; }
...
function objectArt(category: string, accent: string, ink: string) {
  if (category === "smartphones") return phone(accent, ink);
  if (category === "informatique") return laptop(accent, ink);
  ...
  if (category === "editorial") return `${scooter(accent, ink)}<g transform="translate(500 -60) scale(.55)">${phone("white", accent)}</g>`;
...
export async function GET(_: Request, { params }: { params: Promise<{ category: string; variant: string }> }) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
    ...
    <text x="${width - 60}" y="${height - 120}" text-anchor="end" fill="${primary}" font-family="Arial, sans-serif" font-weight="900" font-size="${hero ? 36 : 24}">0${variantNumber} / 26</text><text x="60" y="${height - 65}" fill="white" fill-opacity=".55" font-family="Arial, sans-serif" font-size="${hero ? 22 : 16}" letter-spacing="3">LA TECH QUI VOUS FAIT AVANCER</text>
  </svg>`;
